function jacobian_rhs_simple(
    ...
    Z = zeros(vars_num.N,vars_num.N);            # matrix filled with zeros
    ...
    A11 = C*D; A13 = Diagonal(sin.(psi)); A14 = D*r; b1 = -(C*D*r-cos.(psi));
    
    # determine z from psi 
    A22 = C*D; A23 = Diagonal(-cos.(psi)); A24 = D*z; b2 = -(C*D*z-sin.(psi));
    
    # determine psi from Laplace law
    A31 = -params_phys.sigma*Diagonal(sin.(psi)./r.^2);
    A32 = params_phys.grav*params_phys.deltarho*Diagonal(ones(vars_num.N));
    A33 = C*params_phys.sigma*D + params_phys.sigma*Diagonal(cos.(psi)./r);
    ...
    A41 = reverse(IDL); b4 = (params_phys.rneedle-r[end]);
